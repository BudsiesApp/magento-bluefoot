...
define(['bluefoot/cms-config', 'bluefoot/config', 'bluefoot/jquery', 'bluefoot/dropzone', 'bluefoot/hook', 'bluefoot/modal'], function (InitConfig, Config, jQuery, Dropzone, Hook, Modal) {
        ...
        jQuery(element).dropzone({
            ...
            init: function () {
                ...
                this.on('error', function (file, errorMessage) {
                    Modal.alert('There was an error whilst trying to upload your audio file:<br />' + errorMessage);
                });
    ...
    UploadField.prototype.buildViewHtml = function () {
        jQuery('#audio-container audio').remove();
        if (this.value) {
            this.url = Config.getPluginConfig('gene_widget_upload', 'media_url') + this.value;
            this.audioHtml = '<audio controls>' +
                '<source id="audio-file-src" src="' +this.url+' " type="audio/mpeg">' +
                'Your browser does not support the audio element.</audio>';
        } else {
            this.audioHtml = '<div id="audio-container"></div>';
        }
    ...
    UploadField.prototype.setValue = function (value) {
        if (value !== '' && value != false) {
            this.audioContainer = jQuery('#audio-container').find('audio').remove();
            jQuery(this.audioContainer).find('audio').remove();
            this.url = Config.getPluginConfig('gene_widget_upload', 'media_url') + value;
            this.audioHtml = '<audio id="audio-container" controls>' + '<source id="audio-file-src" src="' +this.url+' " type="audio/mpeg">' + 'Your browser does not support the audio element.</audio>';
            this.value = value;
            this.hiddenInput.val(value);
            jQuery('#audio-container').append(this.audioHtml);
