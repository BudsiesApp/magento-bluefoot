...
/**
 * Override the core varian class to allow us to effectively implement our new button
 *
 * Class Varien_Data_Form_Element_Editor
 *
 * This class is in the wrong location, but will still be loaded correctly thanks to our declare auto loader present
 * in the Observer.php file within Stage.
 *
...
{
    /**
     * Gene_BlueFoot_Model_Stage_Form_Element_Editor constructor.
     *
    ...
    public function getElementHtml()
    {
        ...
        if ($this->isEnabled()) {
            ...
            $html = $this->_getButtonsHtml()
                . '<div class="gene-bluefoot-stage-container" id="' . $this->getStageHtmlId() . '" style="display: none;"></div>'
            ...
            if ($this->getConfig('widget_window_url')) {
                $html = $this->_getButtonsHtml() . $js;

                // If the stage is enabled include it's HTML
                if ($this->isStageEnabled()) {
                    ...
                    //else: include the original textarea
    ...
    {
        return Mage::helper('gene_bluefoot/stage')->isStageEnabled();
    }
    ...
    protected function _getButtonsHtml()
    {
        ...
        // Include our page builders button
        if ($this->isStageEnabled()) {
            $buttonsHtml .= $this->_getStageButtonsHtml();
        }
    ...
    /**
     * Return HTML button to toggling Gene Page Builder
     *
     * @param $visible
     *
     * @return string
     */
    protected function _getStageButtonsHtml($visible = true)
    {
        $html = $this->_getButtonHtml(array(
                'title' => Mage::helper('gene_bluefoot/stage')->getButtonLabelHtml(),
                'class' => 'gene-bluefoot init-gene-bluefoot',
                'id'    => 'gene-bluefoot' . $this->getHtmlId(),
            )) .
            $this->_getButtonHtml(array(
                'title' => '<i class="fa fa-minus-square"></i> ' . $this->translate('Disable BlueFoot'),
